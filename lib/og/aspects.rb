...
module Aspects
  ...
  class Advice # :nodoc: all
    ...
    def wrap(klass, m)
      ...
      class_instance_methods = klass.public_instance_methods(true) +
                               klass.protected_instance_methods(true) +
                               klass.private_instance_methods(true)
      if class_instance_methods.include? m.to_s
        key = "#{klass}.#{m}"
    ...
    def apply(klass)
      ...
      for a in klass.advices
        if a.targets == :LOCAL_METHODS
          meths = klass.public_instance_methods(false) #instance_interface(:local, :public)
        else
          meths = a.targets
        end
        for m in meths
